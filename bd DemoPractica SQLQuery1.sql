...
Insert Cliente Values(47660746,'3','Vásquez Ventura','Juan','Urb. Santa Rosa','996687349')
Go
...
As Begin
	Select IdCliente,Rut,Apellidos,Nombres,Direccion,Telefono From Cliente 
   End
Go
...
As Begin
	If(Exists(Select * From Cliente Where Rut=@Rut))
		Set @Mensaje='Los Datos del Cliente ya Existen.'
	Else Begin
		Insert Cliente Values(@Rut,@dv,@Apellidos,@Nombres,@Direccion,@Telefono)
...
@Datos Varchar(80)
As Begin
	Select IdCliente,Rut,Apellidos,Nombres,Direccion,Telefono 
...
As Begin
	If(Not Exists(Select * From Empleado Where Rut=@Rut))
		Begin
		Insert Empleado Values(@IdCargo,@Rut,@dv,@Apellidos,@Nombres,@Sexo,@FechaNac,@Direccion,@EstadoCivil)
			Set @Mensaje='Registrado Correctamente Ok.'
		End
	Else Begin
	If(Exists(Select * From Empleado Where Rut=@Rut))
		Begin
		Update Empleado Set IdCargo=@IdCargo,Rut=@Rut,Apellidos=@Apellidos,Nombres=@Nombres,Sexo=@Sexo,
...
As Begin
	Select E.IdEmpleado,E.IdCargo,CONCAT(E.Rut,'-',E.DV) AS Rut,E.Apellidos,E.Nombres,E.Sexo,E.FechaNac,E.Direccion,E.EstadoCivil,C.Descripcion From Cargo C Inner Join Empleado E On C.IdCargo=E.IdCargo
End
Go
...
Go
Insert Empleado Values(1,34004387,'1','Silva Terrones','Miguel','M','11/01/1990','Urb. Santa Rosa','S')
Go
